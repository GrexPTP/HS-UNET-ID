                    ...
                    <div class="table-responsive">
                        <table class="table align-items-center table-flush">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="sort" data-sort="gender">Role</th>
                                <th scope="col"></th>
                            </tr>
                            </thead>
                            <tbody class="list">
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td>
                                        <div class="avatar-group">
                                            <div class="media-body">
                                                <span class="name mb-0 text-sm">{{ $user->role ? $user->role->name : 'Not found' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="text-right">
                                        <div>
                                            <a class="btn btn-icon bg-success btn-tooltip" type="button" href="{{ route('users.show', $user->id) }}"
                                               title="Show {{ $user->name }}'s information">
                                                <span class="btn-inner--icon"><i class="white-icon fas fa-eye"></i></span>
                                            </a>
                                            <a class="btn btn-icon bg-info" type="button" href="{{ route('users.edit', $user->id) }}"
                                               title="Edit {{ $user->name }}'s information">
                                                <span class="btn-inner--icon"><i class="white-icon fas fa-edit"></i></span>
                                            </a>
{{--                                            <a class="btn btn-icon bg-danger" type="button" href="{{ route('users.destroy', $user->id) }}">--}}
