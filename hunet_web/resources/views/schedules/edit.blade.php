        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <form action="{{ route('schedules.update', $user->id) }}" method="POST">
                        @method('PATCH')
                        @csrf
                        <div class="card-body">
                                    ...
                                    @for($i = 8; $i < 20; $i++)
                                        ...
                                        <tr>
                                            @for($j = 0; $j < 7; $j++)
                                                ...
                                                <td @if($schedules[$dates[$j]][$i] == 1) class="chosen" @endif>{{ $i . ':00' }} - {{ $i + 1 . ':00' }}<input type="hidden" name="data[{{ $dates[$j] }}][{{$i}}]" value={{$schedules[$dates[$j]][$i]}}></td>
                                            @endfor
                                        </tr>
                                    @endfor
                                    </tbody>
